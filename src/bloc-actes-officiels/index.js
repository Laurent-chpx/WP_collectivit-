...
registerBlockType('wpcollectivites/actes-officiels', {
    ...
    edit: ({ attributes, setAttributes }) => {
        ...
        useEffect(() => {
            apiFetch({ path: '/wp/v2/type-acte?per_page=100' })
                .then((terms) => {
                    const options = [
        ...
        const blockProps = useBlockProps({ className: 'bloc-actes-officiels' });

        return (
            <>
                <InspectorControls>
                    <PanelBody title="Configuration des actes officiels" initialOpen={true}>
                        {isLoading ? (
                            <Spinner />
                        ) : (
                            <SelectControl
